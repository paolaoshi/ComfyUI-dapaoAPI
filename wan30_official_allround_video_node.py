...
NODE_NAME = "DapaoWan30OfficialAllroundVideoNode"
DISPLAY_NAME = "🦁Wan3.0-官方全能视频@炮老师的小课堂"
NODE_CATEGORY = "🤖dapaoAPI/🍬大炮AI主力维护🍬"
MODEL_OPTIONS = ["wan3.0-video-official", "wan3.0-video-prime-official"]
VIDEO_REFERENCE_MODELS = set(MODEL_OPTIONS)
MODE_OPTIONS = ["文生视频", "图生视频", "首尾帧生视频", "多模态参考"]
RESOLUTION_OPTIONS = ["480P", "720P", "1080P"]
ASPECT_RATIO_OPTIONS = ["自适应", "21:9", "16:9", "4:3", "1:1", "3:4", "9:16"]
...
class DapaoWan30OfficialAllroundVideoNode:
    @classmethod
    def INPUT_TYPES(cls):
        optional = {
        ...
        for index in range(1, MAX_VIDEOS + 1):
            optional[f"🎞️ 参考视频{index}"] = (IO.VIDEO, {"tooltip": f"官方 Wan 多段参考视频合计不超过15秒；自动转MP4/H.264/AAC、≤1080p/30fps；提示词写“视频{index}”。"})
    ...
    DESCRIPTION = "官方 Wan 3.0/Prime 文生、图生、首尾帧和多模态参考；使用妙笔官方模型 ID 与 metadata.input.media。"
    ...
    def _safe_response(value):
        ...
        if isinstance(value, dict):
            return {key: ("<敏感信息已隐藏>" if any(part in str(key).lower() for part in secret_keys) else
                          ...
                          if str(key).lower() in media_keys and item else DapaoWan30OfficialAllroundVideoNode._safe_response(item))
                    for key, item in value.items()}
        if isinstance(value, list):
            return [DapaoWan30OfficialAllroundVideoNode._safe_response(item) for item in value]
    ...
    def _generate_sync(self, **kwargs):
        ...
        model = str(kwargs.get("🤖 模型") or MODEL_OPTIONS[0])
        try:
                ...
                for index in range(1, MAX_VIDEOS + 1):
                    ...
                    if video is not None:
                        if model not in VIDEO_REFERENCE_MODELS:
                            raise ValueError("官方 Wan 两个模型均支持参考视频。")
            ...
            media = []
            if mode == "图生视频":
                media.append({"type": "first_frame", "url": image_urls[0]})
            elif mode == "首尾帧生视频":
                media.extend([{"type": "first_frame", "url": image_urls[0]}, {"type": "last_frame", "url": image_urls[1]}])
            else:
                media.extend({"type": "reference_image", "url": url} for url in image_urls)
                media.extend({"type": "reference_video", "url": url} for url in video_urls)
                media.extend({"type": "reference_audio", "url": url} for url in audio_urls)
            parameters = {"resolution": resolution, "ratio": "adaptive" if ratio == "自适应" else ratio,
                          "audio": bool(kwargs.get("🔊 生成音频", True)), "prompt_extend": True, "watermark": False,
                          "seed": cache_seed}
            payload = {"model": model, "prompt": prompt, "seconds": duration, "metadata": {"parameters": parameters}}
            if media:
                payload["metadata"]["input"] = {"media": media}
            ...
            info = (f"✅ Wan3.0官方全能视频任务完成\n🌐 中转站：{API_BASE_URL}\n🤖 模型：{model}\n🎛️ 模式：{mode}\n"
                    f"⏱️ 时长：{duration}\n🧩 分辨率：{resolution}\n📐 比例：{ratio}\n🔊 生成音频：{parameters['audio']}\n"
            ...
            dimensions = {"480P": 854, "720P": 1280, "1080P": 1920}[resolution]
            ratio_sizes = {"21:9": (21, 9), "16:9": (16, 9), "4:3": (4, 3),
                           "1:1": (1, 1), "3:4": (3, 4), "9:16": (9, 16)}
            if ratio in ratio_sizes:
                rw, rh = ratio_sizes[ratio]
                if rw >= rh:
                    width, height = dimensions, round(dimensions * rh / rw)
                else:
                    width, height = round(dimensions * rw / rh), dimensions
            else:
...
NODE_CLASS_MAPPINGS = {NODE_NAME: DapaoWan30OfficialAllroundVideoNode}
