...
const NODE_TYPE = "DapaoSeedreamV5ProAllroundNode";
...
function ensureRegisterButton(node) {
    if (!node?.addCustomWidget || node.__dapaoSeedreamV5RegisterWidget) return;
    ...
    const registerWidget = {
        ...
        type: "DAPAO_SEEDREAM_V5_REGISTER_BUTTON",
        serialize: false,
        _hovered: false,
        _area: null,
        computeSize() {
    ...
    node.__dapaoSeedreamV5RegisterWidget = registerWidget;
}

function refreshNode(node) {
    if (nodeType(node) !== NODE_TYPE) return;
    const transparent = Boolean(value(node, "🫧 透明背景", false));
    setComboValues(widget(node, "📦 输出格式"), transparent ? ["PNG"] : ["JPEG", "PNG"], "PNG");
    const count = Math.max(1, Number(value(node, "🖼️ 出图数量", 1)) || 1);
    setWidgetHidden(node, "⚡ 多图并发", count <= 1);
...
function wrapCallback(node, target) {
    if (!target || target.__dapaoSeedreamV5Wrapped) return;
    ...
    target.__dapaoSeedreamV5Wrapped = true;
}
...
app.registerExtension({
    name: "Dapao.SeedreamV5ProAllround.UI",
    async setup() {
...
console.log("[Dapao Seedream V5 Pro Allround UI] loaded");
