...
const NODE_TYPE = "DapaoGPTLLMChatNode";
...
function ensureRegisterButton(node) {
    if (!node?.addCustomWidget || node.__dapaoGPTLLMRegisterWidget) return;
    ...
    const registerWidget = {
        ...
        type: "DAPAO_GPT_LLM_REGISTER_BUTTON",
        serialize: false,
        _hovered: false,
        _area: null,
        computeSize(width) {
            return [Math.max(160, width), 38];
        },
    ...
    node.__dapaoGPTLLMRegisterWidget = registerWidget;
}
...
function wrapCallback(node, target) {
    if (!target || target.__dapaoGPTLLMWrapped) return;
    ...
    target.__dapaoGPTLLMWrapped = true;
}
...
console.log("[Dapao GPT LLM Chat UI] loaded without price badge");
