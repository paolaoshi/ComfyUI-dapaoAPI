...
### 🚀 2026-08-23：dapaoAI Skill 多轮对话工作台与@素材库

“🍬大炮API常用工具🍬”新增五个可组合节点：API模型配置、API对话增强设置、API Skill加载器、API多轮对话素材库和API多轮对话。支持多轮历史、Skill自动/手动路由、按需读取references、快捷选项、重新生成、清空、上下文占用与多模态聊天。

- 默认调用 `https://api.dapaoai.com/v1/chat/completions`，模型候选与项目其他LLM节点统一。
- API Skill加载器优先读取本插件 `skills/`，并兼容同级 `ComfyUI-llama_Dapao/skills/`。
- API Skill加载器支持手动显示名、连接上游API模型一次性优化异常名称，以及上传Skill文件夹/ZIP自动校验安装；显示别名不会改写Skill内容，旧工作流始终按稳定ID兼容。
- Skill安装支持标准单目录和 `仓库/skills/*` 完整仓库包；ZIP会拦截路径穿越、符号链接、重复路径和超限内容，同名Skill不自动覆盖。详细格式见 `skills/README.md`。
- 每张聊天图片在真正请求前独立做Lanczos等比缩放，最长边不超过2048px，并以PNG发送。
- 素材库最多准备20图、5视频、5音频；只有聊天框本轮明确 `@图片N`、`@视频N`、`@音频N` 后，对应素材才会处理并进入请求，未引用素材不上传。
- 素材只在被引用的当前轮发送；后续需要模型重新查看时请再次 `@`。视频按模型能力抽取代表帧，音频只发送给明确支持音频输入的映射模型。
- 节点内“发送”会直接排队当前聊天节点及其上游依赖，不需要再点击ComfyUI全局运行。
- 聊天操作区增加“清除上下文”和“发送最终状态”：前者保留可见聊天记录但让后续模型从空上下文重新开始，并可在60秒内撤销；后者不调用LLM，只把Skill最终结果（尚无最终结果时回退最近助手回复）按需发送到真实下游。
- “发送最终状态”只排队聊天节点、与它相连的下游及这些下游的必要依赖，不会执行画布上的无关分支；普通“发送”继续隔离全部下游。
- 聊天回复支持安全Markdown、表格与代码块复制；长回复可折叠，输入框可纵向拉伸，并可一键回到底部。
- 会话支持Markdown/JSON导出、JSON恢复和60秒撤销；历史消息可从指定轮次编辑重发或删除后续分支。
- 多轮节点输出助手回复、会话历史JSON和Skill最终结果三个STRING；只有用户点击“发送最终状态”或主动运行完整工作流时才会触发已连接下游。
- 后续运行下游时，即使聊天草稿已经清空，助手回复与Skill最终结果也会从最近历史回填，不会再次请求LLM；真正的Skill最终成品存在时优先输出最终成品。
- 每轮显示输入/输出/总token和调用次数；仅在API明确返回费用时显示金额，否则以dapaoAI后台账单为准。
- API多轮节点支持ComfyUI列表输入的异步并发映射；付费POST不自动重试。
- “自动选择Skill”会额外调用一次当前LLM做确定性路由；固定选择Skill可避免该次调用。
